<template lang="html">
  <div class="remove-items-compartilhados row" v-if="this.$root.credentials">
    <div class="col-md-10 col-md-offset-1 text-left">
      <h2 class="form-title">Rejeitar compartilhamento</h2>
      <h6 class="form-subtitle">Confirme a rejeicao do compartilhamento.</h6>
      ...
      <div>
        ...
        <button type="submit" class="btn btn-danger mt-3" @click="mudarStatus()">Rejeitar o compartilhamento</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    mudarStatus: function() {
      axios.get("http://localhost:9090/api/compartilhamento/" + this.compartilhamento.id + "/rejeitar", this.httpOptions)
        .then(() => {
          this.success = true;
          this.error = false;
